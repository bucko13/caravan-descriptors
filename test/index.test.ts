...
describe("decodeDescriptors", () => {
  ...
  it("should throw if called with inconsistent network", async () => {
    ...
    let passed = false;
    try {
      await decodeDescriptors(testInternal, testExternal, "mainnet");
    } catch (e: any) {
    ...
    passed = false;
    try {
      await decodeDescriptors(internal, external, "testnet");
    } catch (e: any) {
      if (e instanceof Error) {
        passed = true;
        expect(e.message).toMatch("xpubs do not match expected network");
...
describe("getWalletFromDescriptor", () => {
  ...
  it("should fail if passed with inconsistent network", async () => {
    let passed = false;
    try {
      await getWalletFromDescriptor(internal, "testnet");
    } catch (e: any) {
      if (e instanceof Error) {
        passed = true;
        expect(e.message).toMatch("xpubs do not match expected network");
